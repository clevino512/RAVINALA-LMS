...
function CourseProgressChart({ items }) {
    ...
    return (
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h2 className="text-xl font-semibold text-slate-900">Progression moyenne par cours</h2>
                    <p className="mt-1 text-sm text-slate-500">Vue rapide pour repérer les cours qui avancent le mieux.</p>
                </div>
                <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">Graphique</span>
            </div>
            ...
            {items.length === 0 ? (
                ...
                <div className="mt-6 space-y-4">
                    {items.map((item) => {
                        ...
                        const width = `${Math.max((value / highest) * 100, value > 0 ? 8 : 3)}%`;
...
export default function Dashboard({ courses, stats, charts }) {
    return (
        <AppLayout
            header={
                <div>
            ...
            <div className="space-y-6">
                <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                    ...
                    <div className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm">
                        ...
                        <p className="mt-4 text-sm text-slate-500">Progression moyenne</p>
                        <p className="mt-1 text-4xl font-bold text-slate-900">{stats.averageProgress}%</p>
                    </div>
                </section>

                <CourseProgressChart items={charts.courseProgressOverview} />
                        ...
                        {courses.length === 0 ? (
                            ...
                            <article key={course.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                                ...
                                <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-slate-600">
                                    ...
                                    <span className="rounded-full bg-emerald-100 px-3 py-1 text-emerald-700">Moyenne {course.average_progress}%</span>
                                </div>
